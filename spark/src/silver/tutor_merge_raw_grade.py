...
logger = logging.getLogger("silver.grade")

S3_BRONZE_PATH  = "s3a://lakehouse/bronze/tutor/grade"
SILVER_DATABASE = "silver"
SILVER_TABLE    = "tutor_grade"
S3_SILVER_PATH  = "s3a://lakehouse/silver/tutor/grade"
PRIMARY_KEY     = "id"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Silver UPSERT – grade")
...
def ensure_silver_table(spark: SparkSession) -> None:
    ...
    spark.sql(f"""
        ...
        TBLPROPERTIES (
            ...
            'description' = 'Silver raw (UPSERT) for grade'
        )
    """)
...
def main():
    ...
    logger.info("=== Silver pipeline grade | %s → %s ===", start_date, end_date)
