...
logger = logging.getLogger("silver.report_monthly_learning_student_info")

# Bronze source
BRONZE_DATABASE     = "bronze"
BRONZE_TABLE        = "tutor_report_monthly_learning_student_info"
S3_BRONZE_PATH      = "s3a://lakehouse/warehouse/bronze.db/tutor/report_monthly_learning_student_info"

# Silver target
SILVER_DATABASE     = "silver"
SILVER_TABLE        = "tutor_report_monthly_learning_student_info"
S3_SILVER_PATH      = "s3a://lakehouse/warehouse/silver.db/tutor/report_monthly_learning_student_info"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Silver UPSERT – report_monthly_learning_student_info"
    )
    parser.add_argument(
        "--start-date",
        type=str,
...
def build_spark() -> SparkSession:
    spark = (
        ...
        .appName("silver_report_monthly_learning_student_info")
        .enableHiveSupport()
        .getOrCreate()
    )
...
def ensure_silver_table(spark: SparkSession) -> None:
    ...
    spark.sql(f"""
        ...
        TBLPROPERTIES (
            ...
            'description' = 'Silver raw (UPSERT) for report_monthly_learning_student_info'
        )
    """)
...
def main():
    ...
    logger.info(
        "=== Silver pipeline report_monthly_learning_student_info | %s → %s ===",
        start_date, end_date
    )
